...
function makeTabs(categories) {
  if(categories){
    ...
    var searchTerm = 'a';
    var requestUrl = `https://mindreaders.herokuapp.com/api/getproducts?query=${searchTerm}&catId=${tabId}`;
    var tabNav = `
  ...
  $('a[data-toggle="tab"]').on('click', function (e) {
    ...
    var requestUrl = `https://mindreaders.herokuapp.com/api/getproducts?query=${randomQuery}&catId=${catID}`;
  ...
  */
  ...
  function searchNow() {
    if($('#query').val() != '') {
      ...
      // $('#results').empty();
      var url = `https://mindreaders.herokuapp.com/api/getproducts?query=${searchTerm}`;
        fetchAndDisplay(url);
